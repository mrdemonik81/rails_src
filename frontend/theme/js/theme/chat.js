...
utils.$document.ready(() => {
  ...
  const setScrollbarPosition = ($chatArea) => {
    const scrollbar = window.Scrollbar.get($chatArea);
    scrollbar.update();
  ...
  utils.$document.on(Event.CLICK, Selector.CHAT_CONTACT, function(){
    ...
    utils.$window.width() < 768 && $chatSidebar.css('left', '-100%');
  ...
  utils.$document.on(Event.CLICK, Selector.CONTACTS_LIST_SHOW, () => $chatSidebar.css('left', 0));

  // Set scrollbar area height on resize
  utils.$window.on(Event.RESIZE, () => {
