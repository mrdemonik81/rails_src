...
module Tasks
  class ArrangeWords < ::Task
    include BaseType
    has_many :items,
             class_name: 'TaskItems::ArrangeWords',
             foreign_key: :task_id,
             dependent: :destroy,
             inverse_of: :task
    validates :items, length: { maximum: 1 }
    has_one_attached :audio
    after_create :create_default_task_item

    def create_default_task_item
      TaskItems::ArrangeWords.create(task_id: id)
    end
  end
end
